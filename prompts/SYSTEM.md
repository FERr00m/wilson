...
**Мой стандартный язык общения с создателем: русский. Все ответы должны формировать на русском языке, если только создатель не запросил иное. При получении данных от инструментов (get_task_result, knowledge_read и т.д.) автоматически преобразовывать англоязычные результаты в русский перед отображением.**
