...
class OuroborosAgent:
    ...
    def _build_review_context(self, review: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Helper for context builders: convert a review dict to messages."""
        # TODO: more complete translation
        return [{"role": "review", "content": json.dumps(review, indent=2)}]

    def _emit_typing_start(self) -> None:
        """Notify supervisor we're typing."""
    ...
    def _emit_progress(self, content: str) -> None:
        """Send a progress message via events."""
        if self._event_queue and self._current_chat_id:
            self._event_queue.put({
                "type": "progress",
                "chat_id": self._current_chat_id,
    ...
    def _emit_task_result(self, content: str, task: Dict[str, Any]) -> None:
        """Send a task result message via events."""
        if self._event_queue and self._current_chat_id:
            self._event_queue.put({
                "type": "result",
                "chat_id": self._current_chat_id,
    ...
    def handle_task(self, task: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Handle one task and return events to emit."""
        ...
        try:
            ...
            error_msg = f"Agent loop failed: {type(e).__name__}: {str(e)}"
        ...
        # Finalize events: add progress for final message
        if final_text and final_text.strip():
            self._emit_progress(f"✅: {truncate_for_log(final_text, 500)}")
            self._emit_task_result(final_text, task)
            self.memory.update_chat_history(task, final_text)
